...
try {
        ...
        if (user) {
          ...
          if (result) {
            const token =  jwt.sign({'_id':user._id,'userType':user.userType},process.env.secretTokenKey,{ expiresIn: '1h' });
...
const handleUpload = async(req) =>{
    try {
      ...
      const {product,productCategory,productDescription,productQuantity,productPrize,productSupplier} = req.body;
...
const getProductsByFilter = async(req,pageOffset,pageSize)=>{
  try {
    ...
    const FiletedProduct = await products.find(filter)
      .sort(sortOption)
